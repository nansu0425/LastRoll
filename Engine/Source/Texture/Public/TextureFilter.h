...
public:
    ...
    /**
     * @brief 텍스처에 2-pass 분리형 필터를 적용한다.
     * @details
     * 1. 가로(Row) 패스: 입력 텍스처(SRV) -> 내부 임시 텍스처(UAV)
     * 2. 세로(Column) 패스: 내부 임시 텍스처(SRV) -> 출력 텍스처(UAV)
     *
     * @note 입력 텍스처와 출력 텍스처의 크기(너비/높이)는 동일해야 한다.
     * @note 내부 임시 텍스처는 입력 텍스처의 크기에 맞춰 필요에 따라 자동으로 리사이즈된다.
     *
     * @param InTexture 필터링할 원본 텍스처의 셰이더 리소스 뷰 (SRV).
     * @param OutTexture 필터링 결과를 저장할 텍스처의 순서 없는 액세스 뷰 (UAV).
     * @param ThreadGroupCountX 디스패치할 X축 스레드 그룹 수.
     * @param ThreadGroupCountY 디스패치할 Y축 스레드 그룹 수.
     * @param ThreadGroupCountZ 디스패치할 Z축 스레드 그룹 수.
     */
    void FilterTexture(ID3D11ShaderResourceView* InTexture, ID3D11UnorderedAccessView* OutTexture,
        uint32 ThreadGroupCountX, uint32 ThreadGroupCountY, uint32 ThreadGroupCountZ);

    /**
     * @brief 텍스처에 2-pass 분리형 필터를 적용한다 (Row/Column 단위 특화).
     * @details
     * 이 오버로드는 Summed Area Table (SAT) 생성과 같이 Row/Column 전체를
     * 스캔하는 방식의 필터링에 사용된다.
     *
     * 1. 첫 번째 패스 (예: Column 스캔): `Dispatch(1, ThreadGroupCount, 1)`을 호출한다.
     * 2. 두 번째 패스 (예: Row 스캔): `Dispatch(ThreadGroupCount, 1, 1)`을 호출한다.
     *
     * @note `ThreadGroupCount`는 첫 번째 패스의 Y축 그룹 수와 두 번째 패스의 X축 그룹 수로
     * 공통적으로 사용된다.
     * @note 입력 텍스처와 출력 텍스처의 크기(너비/높이)는 동일해야 한다.
     * @note 내부 임시 텍스처는 입력 텍스처의 크기에 맞춰 필요에 따라 자동으로 리사이즈된다.
     *
