...
void UDirectionalLightComponentWidget::RenderWidget()
{
    ...
    if (DirectionalLightComponent->GetCastShadows())
    {
		...
		EShadowProjectionMode CurrentProjectionMode = DirectionalLightComponent->GetShadowProjectionMode();

		const char* ProjectionModeNames[] = { "Uniform SM", "CSM" };
		const EShadowProjectionMode ModeValues[] = { EShadowProjectionMode::Uniform, EShadowProjectionMode::CSM };

		int CurrentUIIndex = (CurrentProjectionMode == EShadowProjectionMode::CSM) ? 1 : 0;
		...
		if (ImGui::BeginCombo("투영 방식", CurrentProjectionModeName))
		{
			for (int i = 0; i < IM_ARRAYSIZE(ProjectionModeNames); ++i)
			{
				...
				{
					DirectionalLightComponent->SetShadowProjectionMode(ModeValues[i]);
				}
